...
#ifndef COLORRGBEXP32
#define COLORRGBEXP32
...
#define MAX_MAP_AREA_unsigned charS				(MAX_MAP_AREAS/8)
...
#define MAX_MAP_AREA_unsigned charS				2
...
enum
{
	...
	//         unsigned char for index
	//         if 255, then index = next unsigned char + 255
	//         3 unsigned chars for barycentric coordinates
...
struct dflagslump_t
{
	unsigned __int32 m_LevelFlags;						// LVLFLAGS_xxx
};
...
// Pad a number so it lies on an N unsigned char boundary.
...
// VTMB - Version 17 changed this to 8
// - BKH - Dec/17/2012
...
struct dface_bsp17_t
{
	...
	unsigned char	day[MAXLIGHTMAPS];		// Nightime lightmapping system
	unsigned char	night[MAXLIGHTMAPS];	// Nightime lightmapping system
...
// unsigned char offsets for the PVS and PHS of each cluster, then the raw
...
{
	unsigned char	data[MAX_LIGHTMAPPAGE_WIDTH*MAX_LIGHTMAPPAGE_HEIGHT];
	unsigned char	palette[256*4];
} dlightmappage_t;

typedef struct nameForDatadesc_dlightmappageinfo_t // unnamed structs collide in the datadesc macros
{
	unsigned char			page;			// lightmap page [0..?]
	unsigned char			offset[2];		// offset into page (s,t)
	unsigned char			pad;			// unused
